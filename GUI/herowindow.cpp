...
void heroWindow::on_addButton_clicked() //diverse aggiunte rispetto ai precendeti add
{
    ...
    if(ah->exec() == QDialog::Accepted){
        ...
        if(tmp.getLevel()<FullLvSkill){
            QMessageBox::information(this,tr("Warning"),tr("The level of this Hero isn't enougth to contail all this skill (required lv %1)").arg(FullLvSkill));
            return;
        }
        ...
        QMessageBox::information(this, tr("Warning"),tr("%1 is in the list").arg(tmp.getName()));
    }
    delete ah;
    return;
}
...
bool heroWindow::firstCheck(char control){
   if(control == 'c' && h == def){ //c per check
       QMessageBox::information(this,tr("Warning"),tr("Select or create an hero"));
        return false;
    }
...
void heroWindow::on_powButton1_clicked()
{
    if(firstCheck('f')){
        timeWindow* tw = new timeWindow;
        QString res1;
